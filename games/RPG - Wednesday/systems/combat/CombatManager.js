...
export class CombatManager {
    ...
    async runPartyMemberTurn(characterId) {
        ...
        // Check stun
        if (this.gameState.isCharacterStunned(characterId)) {
            await this.showNarration(
                `${character.name} is stunned and cannot act!`,
                this.narratorLabel
            );
            this.flow.advanceTurn();
            await this.nextTurn();
    ...
    async runEnemyTurn() {
        ...
        // Check stun
        if (this.gameState.isEnemyStunned()) {
            await this.showNarration(
                `${this.enemyData.shortName} is stunned and cannot act!`,
                this.narratorLabel
            );
            this.flow.advanceTurn();
            await this.nextTurn();
    ...
    async resolveEnemyAction(decision) {
        ...
        EventBus.emit(COMBAT_EVENTS.ACTION.EXECUTED, {
            ...
            abilityId: attack.id,
            ability: attack,
            actionType: attack.damage ? 'attack' : 'buff'
        });
        ...
        for (const target of targets) {
            ...
            if (attack.damage) {
                ...
                if (finalDamage > 0) {
                    ...
                    // Check for Static Shield counter-attack
                    await this.checkStaticShieldCounter(target.id, 'enemy');
                }
            }
    ...
    proceedToRoll(ability) {
        ...
        const accuracyBonus = this.gameState.getPartyAccuracyBonus();
        const hasteBonus = this.gameState.getCharacterHasteBonus(this.selectedCharacter.id);

        EventBus.emit(COMBAT_EVENTS.DICE.ROLL_REQUESTED, {
            stat: ability.stat,
            bonus: statBonus + accuracyBonus + hasteBonus,
    ...
    async checkStaticShieldCounter(targetId, attackerId) {
        if (this.gameState.isCharacterProtectedByStaticShield(targetId)) {
            const counterDamage = 1;
            const target = this.gameState.getMember(targetId);
            
            await this.showNarration(
                `${target.name}'s Static Shield crackles, shocking the attacker!`,
                this.narratorLabel
            );
            
            this.gameState.applyDamageToEnemy(counterDamage);
            EventBus.emit(COMBAT_EVENTS.ENEMY.DAMAGE_FLASH, { amount: counterDamage });
            EventBus.emit(COMBAT_EVENTS.ENEMY.STATUS_UPDATED, { enemy: this.gameState.combat.enemy });
        }
    }
    ...
    async resolvePlayerAction(result) {
        ...
        EventBus.emit(COMBAT_EVENTS.ACTION.EXECUTED, {
            ...
            ability: ability,
            actionType: ability.damage ? 'attack' : 'buff',
            result: result // Pass the dice result (tier) for visuals
        });
        ...
        if (ability.damage && result.tier !== 'failure') {
            ...
            for (let i = 0; i < hits; i++) {
                ...
                const hitDamage = Math.max(0, baseDamage + modifier + (isFirstHit ? markBonus : 0));
                
                if (hitDamage > 0) {
                    this.gameState.applyDamageToEnemy(hitDamage);
                    EventBus.emit(COMBAT_EVENTS.ENEMY.DAMAGE_FLASH, { amount: hitDamage });
                    EventBus.emit(COMBAT_EVENTS.ENEMY.STATUS_UPDATED, { enemy: this.gameState.combat.enemy });
                }
                
                // Area damage hits companion
                if (ability.targetType === 'area' && this.gameState.combat.companion?.active) {
                    const compModifier = getDamageModifier(this.gameState.combat, 'companion', true);
                    const compDamage = Math.max(0, baseDamage + compModifier);
                    if (compDamage > 0) {
                        this.gameState.applyDamageToCompanion(compDamage);
                    }
                }
                
                if (hits > 1 && i < hits - 1) {
                    // Small delay between hits for visual clarity
                    await new Promise(resolve => this.scene.time.delayedCall(200, resolve));
                }
            }
        ...
        if (ability.effect?.type === 'heal') {
            ...
            if (ability.targetType === 'self') {
                ...
                const target = this.targetResolver.resolve('lowest_health', this.gameState.party, this.gameState.getActiveParty());
                if (target) {
                    this.gameState.applyHeal(target.id, healAmount);
                    EventBus.emit(COMBAT_EVENTS.HEALTH.HEAL_RECEIVED, { 
                        targetId: target.id,
                        amount: healAmount,
                        type: 'heal'
        ...
        if (ability.effect && ability.effect.type !== 'heal') {
            this.applyAbilityEffect(ability.effect, ability.targetType);
        }

        if (ability.effects) {
            for (const effect of ability.effects) {
                this.applyAbilityEffect(effect, effect.targetType || ability.targetType);
            }
        }
    }

    applyAbilityEffect(effect, targetType) {
        if (effect.type === 'heal') return; // Handled separately

        if (targetType === 'enemy' || targetType === 'enemy_all') {
            this.gameState.applyEffectToEnemy(effect);
        } else if (targetType === 'party' || targetType === 'area') {
            this.gameState.applyPartyEffect(effect);
        } else if (targetType === 'self') {
            this.gameState.applyEffectToCharacter(this.selectedCharacter.id, effect);
        } else if (targetType === 'ally') {
            const target = this.targetResolver.resolve('lowest_health', this.gameState.party, this.gameState.getActiveParty());
            if (target) {
                this.gameState.applyEffectToCharacter(target.id, effect);
            }
        } else if (targetType === 'all') {
            // Special case for abilities that hit everyone
            this.gameState.applyEffectToEnemy(effect);
            this.gameState.applyPartyEffect(effect);
        }
    }
    ...
    async applyDoTEffects() {
        const enemy = this.gameState.combat?.enemy;
        if (enemy && enemy.currentHealth > 0) {
            const poisonEffects = (enemy.activeEffects || []).filter(e => e.type === 'poison');
            for (const effect of poisonEffects) {
                const dotDamage = effect.damage || 1;
                this.gameState.applyDamageToEnemy(dotDamage);
                
                await this.showNarration(
                    `${enemy.shortName} takes ${dotDamage} damage from ${effect.description || 'poison'}!`,
                    this.narratorLabel
                );
                
                EventBus.emit(COMBAT_EVENTS.ENEMY.DAMAGE_FLASH, { amount: dotDamage });
                EventBus.emit(COMBAT_EVENTS.ENEMY.STATUS_UPDATED, { enemy: this.gameState.combat.enemy });
            }
        }
