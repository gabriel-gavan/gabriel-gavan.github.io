...
const MobileUtils = {
  ...
  createMobileUI() {
    ...
    // Camera Rotation Area (Right Side)
    const rotationPad = document.createElement('div');
    rotationPad.id = 'camera-rotation-pad';
    rotationPad.style.cssText = `
      position: absolute;
      top: 0;
      right: 0;
      width: 60%;
    ...
    return { container, joystickContainer, joystickKnob, jumpButton, rotationPad };
  },
...
class VirtualJoystick {
  constructor(container, knob, onInputChange) {
    ...
    this.isActive = false;
    this.center = { x: 75, y: 75 }; // Center of joystick (half of 150)
  ...
  setupEvents() {
    const handleStart = (e) => {
      e.preventDefault();
      this.isActive = true;
      this.container.style.background = 'rgba(0, 255, 255, 0.3)';
    };
    ...
    const handleMove = (e) => {
      ...
      if (distance <= this.maxDistance) {
        this.knob.style.transform = `translate(${deltaX - 30}px, ${deltaY - 30}px)`;
        ...
        this.knob.style.transform = `translate(${limitedX - 30}px, ${limitedY - 30}px)`;
    ...
    const handleEnd = (e) => {
      ...
      this.container.style.background = 'rgba(0, 255, 255, 0.1)';
...
class MobileControls {
  ...
  setVisibility(visible) {
    ...
    this.mobileUI.container.style.display = visible ? 'block' : 'none';
  }
  ...
  handleJoystickInput(input) {
    ...
    // Directly set analog input on the controller for smoother movement
    this.controller.analogInput.x = input.x;
    this.controller.analogInput.y = input.y;
  }
