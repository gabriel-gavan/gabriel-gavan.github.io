...
class PlayerController {
  constructor(player, options = {}) {
    ...
    this.analogInput = { x: 0, y: 0 }; // For mobile analog movement
  ...
  update(deltaTime, cameraRotation) {
    ...
    // Add analog joystick input (if any)
    if (this.analogInput.y !== 0 || this.analogInput.x !== 0) {
      moveX += forward.x * this.analogInput.y + right.x * this.analogInput.x;
      moveZ += forward.z * this.analogInput.y + right.z * this.analogInput.x;
    }
    ...
    if (moveDirection.lengthSq() > 0) { // Check if there's any horizontal movement input
        if (moveDirection.lengthSq() > 1) moveDirection.normalize(); // Only normalize if over 1 (keys), keep analog intensity
    }
...
class ThirdPersonCameraController {
  ...
  setupMouseControls() {
    ...
    const touchTarget = document.getElementById('camera-rotation-pad') || this.domElement;
    ...
    document.addEventListener('touchmove', (e) => {
      ...
      this.rotation -= deltaX * this.rotationSpeed * 2.0;
