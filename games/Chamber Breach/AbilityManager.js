...
export class AbilityManager {
    ...
    acquire(enemyType) {
        const data = this.abilityData[enemyType];
        if (data) {
    ...
    updateHUD() {
        ...
        if (current) {
            if (this.nameVal) this.nameVal.innerText = `${current.icon} ${current.name} [${this.activeIndex + 1}/3]`;
            if (this.bar) this.bar.style.backgroundColor = `#${current.color.toString(16).padStart(6, '0')}`;
        }
    }
    ...
    update(deltaTime) {
        ...
        if (current && this.bar) {
            ...
            this.bar.style.width = cd > 0 ? `${(cd / current.cooldown) * 100}%` : '0%';
        }
